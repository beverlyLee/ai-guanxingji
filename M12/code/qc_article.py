...
ARTICLE = ROOT / "M12_支持向量机_诈骗短信.md"
...
def main():
    ...
    if not title_match:
        ...
        if "支持向量机" not in title:
            issues.append("标题未出现知识点关键词：支持向量机")
    ...
    math_chars = re.findall(r"[σ²³⁰¹⁴⁵⁶⁷⁸⁹βαθπ∑∫√≈≠≤≥γξ]|\$", text)
    ...
    # 允许的例外：常见计数、折数、章节号、描述性区间、及因四舍五入写入正文的大数
    allowed = set(range(1, 13)) | {0.17, 0.18, 3.1, 309.1, 86.2, 7.1}
    missing = []
    for n in art_nums:
        if n in allowed:
            continue
